...
function parseSectionItems(html, sectionName) {
  // Turkish Wikipedia headings currently use <h2 id="Olaylar">Olaylar</h2>.
  // Support this and older <span> based markup.
  const h2Re = /<h2\b[^>]*>([\s\S]*?)<\/h2>/gi;
  let match;
  while ((match = h2Re.exec(html))) {
    const headingText = cleanHtml(match[1]).trim();
    if (headingText !== sectionName) continue;
    const start = h2Re.lastIndex;
    const next = h2Re.exec(html);
    const sectionHtml = next ? html.slice(start, next.index) : html.slice(start);
    const items = [];
  ...
  for (let attempt=1; attempt<=maxAttempts; attempt++) {
    try {
      const res = await fetch(api, {headers:{
        'User-Agent':'Pratiksel-TarihteBugun/1.2 (personal project; respectful API client)',
...
for(let month=1;month<=12;month++){
  ...
  for(let day=1;day<=maxDay;day++){
    const key=`${month}-${day}`;
    if(cache[key]?.ok && Array.isArray(cache[key].records) && cache[key].records.length > 0){
