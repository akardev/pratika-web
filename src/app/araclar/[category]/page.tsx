...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `/araclar/${category.slug}`,
    },
    openGraph: {
      title: `${pageTitle} | Pratika`,
      description: metaDescription,
      url: `${siteConfig.url}/araclar/${category.slug}`,
      type: 'website',
...
export default async function CategoryPage({ params }: Props) {
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer: 'Kullanıcı geri bildirimleriyle sürekli yeni araçlar ekliyoruz. İletişim sayfamızdan eklenmesini istediğiniz aracı bize doğrudan önerebilirsiniz.',
    },
  ];
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            position: 2,
            name: 'Araçlar',
            item: `${siteConfig.url}/araclar`,
          },
          {
            '@type': 'ListItem',
            position: 3,
            name: category.title,
            item: `${siteConfig.url}/araclar/${category.slug}`,
          },
        ],
      },
      {
