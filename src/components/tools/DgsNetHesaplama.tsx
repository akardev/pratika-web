...
export default function DgsNetHesaplama() {
  const [sayisal, setSayisal] = useState<SubjectInput>({ correct: '38', incorrect: '4' }); // Sayısal 50
  const [sozel, setSozel] = useState<SubjectInput>({ correct: '32', incorrect: '6' }); // Sözel 50
  ...
  return (
    <div className="w-full">
        ...
        <form onSubmit={handleCalculate} noValidate className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 bg-muted/30 rounded-xl border border-border/60 space-y-2">
              <span className="text-sm font-bold text-foreground">Sayısal Bölüm (50 Soru)</span>
              <p className="text-[11px] text-muted-foreground">Matematik ve Geometri</p>
            ...
            <div className="p-4 bg-muted/30 rounded-xl border border-border/60 space-y-2">
              <span className="text-sm font-bold text-foreground">Sözel Bölüm (50 Soru)</span>
              <p className="text-[11px] text-muted-foreground">Türkçe ve Mantıksal Akıl Yürütme</p>
              <div className="grid grid-cols-2 gap-2 pt-1">
                <div>
                  <label className="block text-[11px] text-muted-foreground mb-0.5">Doğru</label>
                  <input
        ...
        {result && (
          <div className="mt-8 p-6 bg-muted/20 rounded-xl border border-border shadow-sm animate-in fade-in zoom-in-95 duration-200">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3 text-center">
              DGS Sınav Net Sonucu (100 Soru)
            </h3>

            <div className="flex flex-col items-center justify-center mb-4 text-center">
              <span className="text-xs text-muted-foreground mb-0.5">Toplam DGS Netiniz</span>
            ...
            <div className="mt-4 pt-3 border-t border-border/60 text-center">
              ...
              >
                TYT net hesaplayıcısına gidin &rarr;
              </Link>
            </div>
          </div>
        )}
      ...
      <div className="prose prose-slate max-w-none">
        <h2 className="text-2xl font-bold mb-4 text-foreground">DGS Net Hesaplama Esasları</h2>
        <p className="mb-4 text-muted-foreground">
          Dikey Geçiş Sınavı (DGS) ön lisans mezunlarının lisans programlarına geçişini sağlar. Sınavda 50 Sayısal ve 50 Sözel olmak üzere toplam 100 soru bulunur; 4 yanlış 1 doğruyu götürür.
        </p>
      </div>
    </div>
  );
